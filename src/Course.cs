...
namespace Sunley.Orienteering.PurplePen.File
{
    public class ControlStore : IEnumerable<ControlPoint>
    {
        ...
        private List<ControlPoint> normalControls = new List<ControlPoint>();
        private List<ControlPoint> specialControls = new List<ControlPoint>();
        ...
        public int Count
        {
            get
            {
                if (controlDict.Count == (normalControls.Count + specialControls.Count))
        ...
        public ControlPoint ChooseStart()
        {
            ...
            foreach (ControlPoint c in specialControls)
                if (c.Type == ControlPointType.Start)
                    valid.Add(c);

            return valid[random.Next(valid.Count)];
        }
        public ControlPoint ChooseFinish()
        {
            List<ControlPoint> valid = new List<ControlPoint>();
        ...
        public ControlPoint ChooseNextControlPoint(List<ControlPoint> course)
        {
            ControlPoint last = course.Last();
            ControlPoint previous = course[course.Count - 2];

            Tuple<int, int> distances = ChooseLength();


            List<ControlPoint> valid = new List<ControlPoint>();

            foreach (ControlPoint c in normalControls)
            {
                float dist = DistBtwnCtrls(last, c);

                if (dist >= distances.Item1 && dist < distances.Item2)
                {
                    if (CheckAngle(previous, last, c))
                    {
            ...
            if (valid.Count > 0)
            {
                ControlPoint c = valid[random.Next(valid.Count)];
                return c;
            }
        ...
        public ControlPoint ChooseRandomControl()
        {
            return normalControls[random.Next(normalControls.Count)];
        }
            ...
            for (int i = 1; i < doc.FirstChild.ChildNodes.Count; i++)
            {
                ...
                if (node.Name == "control")
                {
                    ...
                    if (c.Type == ControlPointType.Normal)
                        normalControls.Add(c);
                    else specialControls.Add(c);
        ...
        void CreateXML(List<Course> courses)
        {
            ...
            System.IO.File.WriteAllText(@"text.ppen", text);
